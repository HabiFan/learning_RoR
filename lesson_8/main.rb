...
class MainApp
  ...
  def start_menu
    loop do
      ...
      when 4 then menu_train_route unless trains.empty?
      when 5 then menu_info unless stations.empty?
      when 6 then break
      else
        INFO_LABEL
      end
  ...
  def menu_train
    loop do
      ...
      when 2
        ...
        add_carriage_train(object_select(trains, :@number)) unless trains.empty?
      when 3
        object_select(trains, :@number).drop_carriage unless trains.empty?
      when 4
        puts 'Выберите нужный поезд'
        selected_train = object_select(trains, :@number) unless trains.empty?
        puts 'Выберите нужный вагон'
        selected_carriage = carriage_select(selected_train.carriages) unless selected_train.carriages.empty?
  ...
  def menu_train_route
    ...
    if train.route.nil?
      ...
      loop do
        ...
        when 1
          train.next_route
          puts 'Поезд на конечной станции' if train.next_station.nil?
        when 2
          train.last_route
          puts 'Поезд в начале маршрута' if train.last_station.nil?
        when 3 then break
        else
          INFO_LABEL
        end
      ...
      when 2
        stations.each do |station|
          station.list_trains do |train|
            puts "На станции: #{station.name} находится поезд: #{train.number}, тип: #{train.type}, кол-во вагонов: #{train.carriages.count}"
          end
        end
      when 3 then break
      else
  ...
  def carriage_select(carriages)
    carriages.each_with_index { |_value, index| puts "Вагон №#{index + 1}." }
  ...
  def route_select
    routes.each_with_index do |route, index|
      puts "#{index + 1}. #{route.begin_station.name}..#{route.end_station.name}"
    end
